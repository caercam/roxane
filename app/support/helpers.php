...
function get_the_rating( $post = null ) {
  ...
  return generate_rating_stars( $rating->name, $rating->name . ' − ' . $rating->description );
}

function generate_rating_stars( $rating = '', $title = '' ) {
    ...
    return '<span class="stars" title="' . esc_attr( $title ) . '">' . $stars . '</span>';  
}